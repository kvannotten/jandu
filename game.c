...
void checkConnected(Map* map, Coord coord, Vector* coords) {
  ...
  for (int i = 0; i < loops; i++) {
    ...
    if(x < 0 || x >= FIELD_SIZE || y < 0 || y >= FIELD_SIZE) {
      printf("(%d,%d) falls outside map, skipping...\n", x, y);
      continue;
    }

    if(map->unitField[y][x].level != currentUnit.level) {
      printf("(%d,%d) has wrong level, skipping...\n", x, y);
      continue;
    }

    if(find_value(coords, (Coord){.x = x, .y = y}) != -1) {
      printf("Already have (%d,%d). Skipping...\n", x, y);
      continue;
    }

    if(map->unitField[y][x].level == currentUnit.level) {
      printf("Found (%d,%d), adding to vector\n", x, y);
      Coord c = { .x = x, .y  = y };
      append(coords, c);
